...
) -> Result<models::GetLocations200Response, Error<GetLocationsError>> {
    let uri_str = format!("{}/api/v2/locations", configuration.base_path);
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::GetLocations200Response`"
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::GetLocations200Response`"
                ))));
            }
        }
    } else {
