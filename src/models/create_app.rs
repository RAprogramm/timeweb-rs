...
pub struct CreateApp {
    ...
    /// Путь к директории с индексным файлом. Обязателен для приложений `type:
    /// frontend`. Не используется для приложений `type: backend`. Значение
    /// всегда должно начинаться с `/`.
    #[serde(rename = "index_dir", skip_serializing_if = "Option::is_none")]
    pub index_dir:           Option<String>,
    /// Команда для запуска приложения. Обязательна для приложений `type:
    /// backend`. Не используется для приложений `type: frontend`.
    #[serde(rename = "run_cmd", skip_serializing_if = "Option::is_none")]
    pub run_cmd:             Option<String>,
    /// Системные зависимости.
    #[serde(
        rename = "system_dependencies",
