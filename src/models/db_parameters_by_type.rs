...
/// DbParametersByType : Имена параметров конфигурации, доступных для каждого
/// типа кластера базы данных. Ключ объекта — тип кластера (значение поля `type`
/// при создании кластера), значение — массив имён параметров, которые можно
/// передать в `config_parameters` для кластера этого типа. Наборы параметров
/// различаются между версиями одной СУБД. Значения параметров этот метод не
/// возвращает — рекомендуемые значения можно получить в `GET
/// /api/v1/dbs/default-parameters`.
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct DbParametersByType {
...
impl DbParametersByType {
    /// Имена параметров конфигурации, доступных для каждого типа кластера базы
    /// данных. Ключ объекта — тип кластера (значение поля `type` при создании
    /// кластера), значение — массив имён параметров, которые можно передать в
    /// `config_parameters` для кластера этого типа. Наборы параметров
    /// различаются между версиями одной СУБД. Значения параметров этот метод не
    /// возвращает — рекомендуемые значения можно получить в `GET
    /// /api/v1/dbs/default-parameters`.
    pub fn new(
