...
pub struct DnsRecordV2Data {
    /// Приоритет DNS-записи (для MX и SRV записей).
    #[serde(rename = "priority", skip_serializing_if = "Option::is_none")]
    pub priority:  Option<f64>,
    /// Имя поддомена (только поддомен без основного домена, например `sub` для
    /// `sub.example.com`). Для записей на основном домене это поле отсутствует
    /// в ответе.
    #[serde(
        rename = "subdomain",
        default,
