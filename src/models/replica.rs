...
/// Replica : Реплика кластера базы данных
...
pub struct Replica {
    ...
    #[serde(rename = "status")]
    pub status:   Status,
    /// IP-адрес реплики в локальной сети. Возвращается пустая строка, если
    /// адрес ещё не назначен.
    ...
    pub disk:     Option<Option<Box<models::ReplicaDisk>>>
}

impl Replica {
    /// Реплика кластера базы данных
    pub fn new(id: i32, db_id: i32, status: Status, local_ip: String) -> Replica {
        Replica {
            id,
            db_id,
            status,
